...
for epoch in range(Nepochs):  
    ...
    if epoch%displayrate==0:
        print(f"{epoch}: {np.sqrt(np.sqrt(np.mean(loss[:100])))} {np.sqrt(np.sqrt(loss[0]))} ",end="")
    ...
    selectidx = rng.choice(Nindiv, size=length, replace=False, p=1/loss/s)
    ...
    length = int(len(nonselectidx)*ratio2)
    length2 = int(len(nonselectidx)*(1-ratio2))
    if epoch%nuke==0:
        length=int(len(nonselectidx)*ratio4)
        length2=int(len(nonselectidx)*(1-ratio4))
    children = np.take(nonselectidx, np.arange(length))
    newindividuals = np.take(nonselectidx, np.arange(int(length2),len(nonselectidx)))
    ...
    p4[selectidx]+=np.broadcast_to(rng.normal(0.0,(Nepochs-epoch)/Nepochs/45,size=len(selectidx))[...,None], (len(selectidx),len(rt)) )
    p5[selectidx]+=np.broadcast_to(rng.normal(0.0,(Nepochs-epoch)/Nepochs/45,size=len(selectidx))[...,None], (len(selectidx),len(rt)) )
    p6[selectidx]+=np.broadcast_to(rng.normal(0.0,(Nepochs-epoch)/Nepochs/45,size=len(selectidx))[...,None], (len(selectidx),len(rt)) )
...
np.savetxt(f"./bestindivs/bestindivNORMAL{time.time()}.txt",np.array([p1[0][0],p2[0][0],p3[0][0],p4[0][0],p5[0][0],p6[0][0]]))
